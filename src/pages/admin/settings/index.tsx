...
import {type IUpcomingEventUpdate, upcomingEventSchema} from "~/validation/upcomingEventValidation";
...
export default function AdminEvents() {
  ...
  const handleSubmit =  async (form: IUpcomingEventUpdate) => {
    eventForm.getValues().id ? submitEventUpdate(form).catch(console.error) : submitEventCreate(form).catch(console.error)
  };
                  ...
                  events?.length ?
                    events.map((event, index) => (
                      <TableRow key={index} className="border-slate-800 hover:bg-transparent">
                        ...
                        <TableCell className="font-medium">{event.locationUrl !== '' ? <Link href={event.locationUrl}></Link> : '-'}</TableCell>
                        <TableCell className="font-medium">{event.date.toDateString()}</TableCell>
                        <TableCell className="font-medium">{event.registerUrl ? <Link href={event.registerUrl}></Link> : '-'}</TableCell>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <CalendarInput className="text-slate-400 bg-transparent border-slate-800" {...field} />
                    </FormControl>
                    <FormMessage></FormMessage>
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g: https://maps.app.goo.gl/v6d...." className="text-slate-400 bg-transparent border-slate-800" {...field} />
                    </FormControl>
                    <FormMessage></FormMessage>
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g: https://...." className="text-slate-400 bg-transparent border-slate-800" {...field} />
                    </FormControl>
                    <FormMessage></FormMessage>
                  </FormItem>
                )}
